...
class Survey(http.Controller):
    ...
    def survey_start(self, survey_token, answer_token=None, email=False, **post):
        ...
        if answer_sudo.state == 'new':  # Intro page
            data = {'survey': survey_sudo,
                    'answer': answer_sudo,
                    'page': 0,
                    'total_time_limit': sum(ques.time_limit for ques in survey_sudo.question_and_page_ids) or 0
                    }
    ...
    def survey_display_page(self, survey_token, answer_token, prev=None, **post):
        ...
        #We need to create all answers becasuse we need to compute time_limit
        if answer_sudo.survey_id.questions_time_option == 'each_question':
            for ques in survey_sudo.question_and_page_ids:
                survey_user_input_line = request.env['survey.user_input_line'].sudo().search([('user_input_id','=', answer_sudo.id),('question_id','=', ques.id)])
                if not survey_user_input_line:
                    vals = {'user_input_id': answer_sudo.id,
                            'question_id': ques.id,
                            }
                    survey_user_input_line = survey_user_input_line.sudo().create(vals)
        ...
        if answer_sudo.state == 'new':  # First page
            ...
            data = {
                ...
                'answer': answer_sudo,
                'survey_user_input_line_time': datetime.now(),
                'random': random,
            }


            if page_or_question_key == 'question' and answer_sudo and answer_sudo.survey_id and answer_sudo.survey_id.questions_time_option == 'each_question':
                survey_user_input_line = request.env['survey.user_input_line'].sudo().search([('user_input_id','=', answer_sudo.id),('question_id','=', page_or_question_id.id)])

                if not survey_user_input_line:
                    vals = {'user_input_id':answer_sudo.id,
                            'question_id':page_or_question_id.id,
                            'start_datetime': datetime.now(),
                            }
                    survey_user_input_line = survey_user_input_line.sudo().create(vals)

                elif survey_user_input_line and not survey_user_input_line.start_datetime:
                    survey_user_input_line.write({
                        'start_datetime': datetime.now()
                    })
                data.update({'survey_user_input_line_time': survey_user_input_line.start_datetime,
                             'survey_user_input_line': survey_user_input_line,
                             })
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        if len(errors):
            ...
            vals = {}
            if answer_sudo.is_time_limit_reached or survey_sudo.questions_layout == 'one_page' or (survey_sudo.questions_time_option == 'each_question' and (post['is_last_question'] and post['is_last_question'] == 'True')):
                go_back = False
            ...
            elif survey_sudo.questions_time_option == 'each_question' :
                go_back = False
                next_page, last = request.env['survey.survey'].next_page_or_question(answer_sudo, page_or_question_id, go_back=go_back)
                vals = {'last_displayed_page_id': page_or_question_id}
